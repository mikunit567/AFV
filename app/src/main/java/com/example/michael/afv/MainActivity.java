	...
	private void setUpGridClickListener() {
		mainGrid.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> arg0, View clickedView, int position,
					...
					pageLoadDialog.show();
                    String filepath = gridAdapter.getPropertiesByPosition(position, AFVDatabase.FILE_LOCATION);
					...
					if (defaultHtmlViewer) {
						...
						try {
							Intent i = new Intent(getApplicationContext(),
									ViewActivity.class);
							i.putExtra(AFVDatabase.ORIGINAL_URL, gridAdapter.getPropertiesByPosition(position, AFVDatabase.ORIGINAL_URL));
							i.putExtra(AFVDatabase.TITLE, gridAdapter.getPropertiesByPosition(position, AFVDatabase.TITLE));
							i.putExtra(AFVDatabase.ID, gridAdapter.getPropertiesByPosition(position, AFVDatabase.ID));
							i.putExtra(AFVDatabase.FILE_LOCATION, filepath);
							i.putExtra(AFVDatabase.THUMBNAIL, gridAdapter.getPropertiesByPosition(position, AFVDatabase.THUMBNAIL));
							i.putExtra(AFVDatabase.TIMESTAMP, gridAdapter.getPropertiesByPosition(position, AFVDatabase.TIMESTAMP));
	...
	class ModeCallback implements ListView.MultiChoiceModeListener	{
        ...
        public boolean onActionItemClicked(final ActionMode mode, MenuItem item) {
			...
			switch (item.getItemId()) {
				case R.id.action_rename:
					...
					rename_dialog.setPositiveButton("Rename",
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog, int which) {
								mHelper = new AFVDatabase(MainActivity.this);
								dataBase = mHelper.getWritableDatabase();

								for (Integer position: gridAdapter.selectedViewsPositions) {
									ContentValues values=new ContentValues();
									values.put(AFVDatabase.TITLE, e.getText().toString() );
									dataBase.update(AFVDatabase.TABLE_NAME, values, AFVDatabase.ID + "=" + gridAdapter.getPropertiesByPosition(position, AFVDatabase.ID), null);
								}
				...
				case R.id.action_delete:
					...
					if (gridAdapter.selectedViewsPositions.size() == 1) {
						build.setMessage("Do you want to delete ?\r\n" + gridAdapter.getPropertiesByPosition(gridAdapter.selectedViewsPositions.get(0), AFVDatabase.TITLE));
					} else {
		...
		private class deleteItemsTask extends AsyncTask<Object, Integer, Integer> {
			...
			protected Integer doInBackground(Object[] selectedPositions) {
				dataBase = new AFVDatabase(MainActivity.this).getWritableDatabase();
				
				for (final Object position : selectedPositions) {
					String fileLocation = gridAdapter.getPropertiesByPosition((Integer) position, AFVDatabase.FILE_LOCATION);
					DirectoryHelper.deleteDirectory(new File(fileLocation).getParentFile());
					
					dataBase.delete(AFVDatabase.TABLE_NAME, AFVDatabase.ID + "=" + gridAdapter.getPropertiesByPosition((Integer)position, AFVDatabase.ID), null);
					currentProgress++;
